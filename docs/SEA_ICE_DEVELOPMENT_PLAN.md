...
- **C1 [gate]** ✅ **DONE (2026-07-17)** — Modular **metadata→prior adapter** (`biosnicar/sea_ice/metadata_priors.py`): composable providers → per-class log-priors + parameter priors + hard physical exclusions, with per-source toggling (`prior_sources=`) and a caller channel (`class_priors=`) for context this module has no validated model for yet (location/ice-age/skin-T — the C2–C4 items). `known_month` is now the `season` provider (parameter priors + young-ice exclusion preserved exactly), plus new soft class priors (fresh snow disfavoured in peak melt; ponds/SSL excluded in deep winter) and provenance for A3. Class priors combine with the spectral evidence (OE: log-add to evidence; default: `−2·log-prior` on the classification χ²). Validated: SHEBA summer band-mode classification improved (S2 4-band 15→16/16, L8 13→15/16), spring held 7/7, full-spectrum summer held 16/16. Leverage is regime-dependent — futile against the over-counted raw-spectral likelihood, effective in band mode / with `model_error=True` (few-nat gaps). Soft magnitudes are conservative expert-judgment defaults pending A6 calibration.
