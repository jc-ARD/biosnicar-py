...
- **A2 [gate]** ✅ **DONE**, with a measured limitation — `retrieve_sea_ice(method="oe")` classifies by per-type posterior probability from the model evidence (`class_probabilities`). **2026-07-15 SHEBA SWIR experiment** (`oe_swir_classification_experiment.py`, validation doc §12): on real melt-season spectra the evidence ranking is unreliable (7/12 VIS-only, 2/12 VIS+SWIR vs the default masked method's 12/12 in both) and maximally overconfident (wrong class at p≈1.00). OE classification is not to be trusted until A6/A7 land; candidate repair is hybrid classification (default masked chi-squared selects the winner, OE runs on the winner for posteriors) — an API behavior change requiring downstream (IceNav) coordination.
